...
from dgl_graph.chem import get_mol
from dgl_graph.get_extra_bond_feature import get_extra_bond_feature
import dgl
import torch
...
def SmileToDGLGraph(smile):
    ...
    bond_list = [[],[]]
    for bond in mol.GetBonds():
        ext_bond_feature = get_extra_bond_feature(bond)
        feat = get_bond_features(bond)
        feat = np.concatenate((feat,ext_bond_feature[0],ext_bond_feature[1]),axis=0)
        bond_features.append(feat)
        st, ed = bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()
        bond_list[0].append(st)
        bond_list[1].append(ed)
    t = bond_list[0].copy()
    bond_list[0].extend(bond_list[1])
    bond_list[1].extend(t)
    bond_features = torch.FloatTensor(np.array(bond_features))
    #Connected Features
    g = dgl.graph((bond_list[0],bond_list[1]))
    g.ndata["feat"] = atom_features
    g.edata["feat"] = torch.cat([bond_features,bond_features])
    return g

if __name__=="__main__":
    smile = "c1ccccc1"
    g = SmileToDGLGraph(smile)
    print(g)