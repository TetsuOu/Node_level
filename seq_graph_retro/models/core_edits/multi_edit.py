...
class MultiEdit(nn.Module):
    ...
    def _compute_edit_logits(self, prod_tensors: Tuple[torch.Tensor],
        ...
        if ha is None:
            ...
            ha = torch.zeros(n_atoms, self.config['mpn_size'], device=self.device)
        ...
        ha = F.relu(self.W_vv(ha) + self.W_vc(c_atom))   #式(20)
        # ha[0] = 0
        ...
        for idx in range(max_seq_len):
            ...
            if idx == 0:
                c_mol, edit_logits, ha = self._compute_edit_logits(prod_tensors,
                                            prod_scopes, ha=ha, seq_order=seq_order)
            else:
                _, edit_logits, ha = self._compute_edit_logits(prod_tensors, prod_scopes, ha=ha,seq_order=seq_order)
    ...
    def _compute_edit_stats(self, seq_edit_logits: List[List[torch.Tensor]],
                            ...
                            seq_mask: torch.Tensor,
                            seq_order: torch.Tensor=None) -> Tuple[torch.Tensor]:
        ...
        for idx in range(max_seq_len):# t步  每个权重都一样  第i步出错 和 第j步出错 造成的影响应该不一样 i<j 时 i造成的影响应该更大
    ...
    def train_step(self, seq_tensors: List[Tuple[torch.Tensor]],
                   ...
                   seq_mask: torch.Tensor, seq_order: torch.Tensor = None,**kwargs) -> Tuple[torch.Tensor, Dict]:
        ...
        if seq_order is not None:
            seq_order = self.to_device(seq_order)
        prod_vecs, seq_edit_logits = self(seq_tensors, seq_mask, seq_order)
        seq_loss, seq_acc = self._compute_edit_stats(seq_edit_logits, seq_labels, seq_mask, seq_order)
        metrics = {'loss': seq_loss.item(), 'accuracy': seq_acc.item()}
        return seq_loss, metrics

    def predict(self, prod_smi: str, rxn_class: int = None, max_steps: int = 6, seq_order: int = -1) -> List:
        ...
        with torch.no_grad():
            ...
            seq_order = torch.Tensor([seq_order]).long()
    ...
    def eval_step(self, prod_smi_batch: List[str],
                  core_edits_batch: List[str], seq_order: List[int]=None, rxn_classes: List[int] = None,
        ...
        for idx, prod_smi in enumerate(prod_smi_batch):
            if rxn_classes is None:
                edits = self.predict(prod_smi, seq_order=seq_order[idx])
            else:
                edits = self.predict(prod_smi, rxn_class=rxn_classes[idx], seq_order=seq_order[idx])
