...
class SingleEdit(nn.Module):
    ...
    def _compute_edit_logits(self, graph_tensors: Tuple[torch.Tensor],
        ...
        bond_score_inputs = torch.cat([sum_bonds, diff_bonds], dim=1)      #c_{uv} bond(u,v)
        atom_score_inputs = c_atom.clone()                                #c_u
        ...
        bond_logits = self.bond_score(bond_score_inputs)      #s_{uvk}
        unimol_logits = self.unimol_score(atom_score_inputs)  #s_u
    ...
    def eval_step(self, prod_smi_batch: List[str],
                  core_edits_batch: List[str],seq_order: List[int]=None,
        ...
        if rxn_classes is None:
            edits, accuracy = self.predict(prod_smi_batch, core_edits=core_edits_batch)
        else:
            edits, accuracy = self.predict(prod_smi_batch, rxn_class=rxn_classes, core_edits=core_edits_batch)

        metrics = {'loss': None, 'accuracy': accuracy}
        return loss, metrics

    def predict(self, prod_smis: list, core_edits: list, rxn_class: int = None) -> List:
        ...
        with torch.no_grad():
            prod_graphs = []
            mols = []
            for prod_smi in prod_smis:
                mol = Chem.MolFromSmiles(prod_smi)
                mols.append(mol)
                prod_graphs.append(RxnElement(mol=Chem.Mol(mol), rxn_class=rxn_class))

            prod_tensors, prod_scopes = pack_graph_feats(prod_graphs,
                                                        directed=directed, return_graphs=False,
                                                        use_rxn_class=use_rxn_class)
            bg_inputs = None
            if self.toggles.get("propagate_logits", False):
                bg_inputs = tensorize_bond_graphs(prod_graphs, directed=directed,
                                                use_rxn_class=use_rxn_class)
                # print(bg_inputs)                                
            _, edit_logits = self(prod_tensors, prod_scopes, bg_inputs)
            pre_edits =[]
            for resultdx, edit_logit in enumerate(edit_logits):
                idx = torch.argmax(edit_logit)
                val = edit_logit[idx]
                mol = mols[resultdx]
                # print(val)
                ...
                if idx.item() < max_bond_idx:
                    if self.config['bs_outdim'] > 1:
                        bond_logits = edit_logit[:mol.GetNumBonds() * len(BOND_FLOATS)]
                    ...
                    h_logits = edit_logit[max_bond_idx:]
                    ...
                    edit = f"{a1}:{0}:{1.0}:{0.0}"
                pre_edits.append([edit])

                if set([edit]) == set(core_edits[resultdx]):
                    accuracy += 1.0
        return pre_edits, accuracy
        # return [edit]
