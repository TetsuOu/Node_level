...
ReactionInfo = namedtuple("ReactionInfo", ['rxn_smi', 'core', 'core_edits', 'lg_edits', 'attach_atoms', 'rxn_class', 'MainId', 'Order'])
...
def get_reaction_info(rxn_smi: str, kekulize: bool = False, use_h_labels: bool = False,
                      rxn_class: int = None, mainid:str = 'xxxxxx', order: int=-1) -> ReactionInfo:
    """
...
def extract_leaving_groups(mol_list: List[Tuple[Chem.Mol]], args=None) -> Tuple[Dict, List[str], List[Chem.Mol]]:
    ...
    for mol_tuple in mol_list:
        ...
        for i, mol in enumerate(reac_mols):
            ...
            if len(idxs):
                ...
                if lg not in lg_mols_cnt_map:
                    lg_mols_cnt_map[lg] = 1
                else:
                    lg_mols_cnt_map[lg] += 1
    ...
    if args is not None:
        lg_mols_cnt_map_file = str(args.data_dir) + "/lg_mol_cnt_map_" + str(args.mode) + ".json"
        json.dump(lg_mols_cnt_map, open(lg_mols_cnt_map_file, "w"), indent=2)
    # print("lg_mols_cnt_map:", lg_mols_cnt_map, file=open("/data/xjx/raw_datasets/lg_mol_cnt_map.txt", "w"))
    # print("parse finish")
    lg_dict = {lg: idx for idx, lg in enumerate(leaving_groups)}
    # print("lg_mols_cnt_map:{}".format(lg_mols_cnt_map))
